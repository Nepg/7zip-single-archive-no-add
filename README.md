# 7zip-single-archive-no-add

一个修改版 7-Zip Shell 扩展：在资源管理器中只选中 1 个压缩包文件时，隐藏右键菜单中的压缩相关项。
...
本项目基于 7-Zip 官方源码，修改了 Shell 扩展的菜单显示逻辑：当只选中 1 个文件，且该文件扩展名属于压缩包时，不插入上述菜单项。
...
1. 从 [Releases](https://github.com/Nepg/7zip-single-archive-no-add/releases) 下载 `7zip-single-archive-no-add-v1.0.zip`。
2. 解压。
3. 关闭所有资源管理器窗口。
...
- 仅 x64。
- 基于 7-Zip 官方源码修改，非官方发布。
- 默认安装路径为 `C:\Program Files\7-Zip\`。
...
A modified 7-Zip Shell extension: hides compress-related context menu items when exactly one archive file is selected in Explorer.
...
This project modifies the Shell extension menu logic based on the official 7-Zip source: when exactly one file is selected and its extension belongs to an archive format, those menu items are not inserted.
...
1. Download `7zip-single-archive-no-add-v1.0.zip` from [Releases](https://github.com/Nepg/7zip-single-archive-no-add/releases).
2. Extract.
...
- x64 only.
- Modified build based on 7-Zip official source, not an official release.
- Default install path is assumed to be `C:\Program Files\7-Zip\`.
...
Based on 7-Zip official source. Follow the original 7-Zip license (GNU LGPL / BSD / unRAR restriction).
